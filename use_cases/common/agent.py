...
# ── Phase 3 vision constants ──────────────────────────────────────────────────
# All overrideable via .env

# Seconds to wait for Nova Pro assess() before treating result as "continue"
VISION_ASSESS_TIMEOUT = float(os.getenv("VISION_TIMEOUT",   "4.0"))

# Maximum vision-triggered replans allowed per execution run
MAX_REPLANS           = int(os.getenv("MAX_REPLANS",         "3"))

# Steps shorter than this are skipped for vision assessment
# (0.3 s stabilisation steps don't warrant a full Nova Pro API call)
VISION_MIN_STEP_SECS  = float(os.getenv("VISION_MIN_STEP",  "0.5"))

# Lazy re-export of Phase 3 types so callers can do `from agent import AssessContext`.
# Definitions live in vision_assessor.py to keep the import graph acyclic.
# Falls back to None in Phase 2 standalone mode (vision_assessor.py absent).
def _try_import_vision_types():
    try:
        from vision_assessor import AssessContext, VisionDecision  # noqa: F401
        return AssessContext, VisionDecision
    except ImportError:
        return None, None

AssessContext, VisionDecision = _try_import_vision_types()
